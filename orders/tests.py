...
class OrderModelTest(TestCase):
    ...
    def test_deposit_amount_is_50_percent(self):
        self.assertEqual(self.order.deposit_amount, Decimal('5000.00'))

    def test_remaining_balance_at_checkout(self):
        self.assertEqual(self.order.remaining_balance, Decimal('10000.00'))
...
class BuyNowViewTest(TestCase):
    ...
    def test_product_marked_sold_after_purchase(self):
        self.client.login(username='buyer', password='pass1234')
        self.client.post(reverse('marketplace:buy_now', kwargs={'pk': self.product.pk}))
...
class KhaltiPaymentTest(TestCase):
    ...
    def test_initiate_sends_50_percent_in_paisa(self, mock_post):
        ...
        deposit_50_percent = self.order.price_at_purchase * Decimal('0.50')
        self.assertEqual(payload['amount'], int(deposit_50_percent * 100))
    ...
    @patch('orders.views.requests.post')
    def test_initiate_50_percent_rounds_to_paisa(self, mock_post):
    ...
    @patch('orders.views.requests.post')
    def test_verify_completes_split_payment(self, mock_post):
        self.client.login(username='pay_buyer', password='pass1234')
        ...
        self.assertEqual(response.status_code, 302)
        self.order.refresh_from_db()
        self.assertEqual(self.order.amount_paid, Decimal('2500.00'))
